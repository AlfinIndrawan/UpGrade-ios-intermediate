...
class LoginViewController: UIViewController {
  ...
  var loginStatus: LoginStatus = .signUp {
    didSet {
      ...
      self.accessoryBtn.setTitle((loginStatus == .signUp) ? "Already have account?" : "Don't have account?", for: .normal)
  ...
  @IBAction func primaryBtnAction(_ sender: Any) {
    if emailIsEmpty || passwordIsEmpty {
      let alert = UIAlertController(title: NSLocalizedString("Missing Information", comment: ""), message: NSLocalizedString("Please make sure to enter a valid email password", comment: ""), preferredStyle: .alert)
